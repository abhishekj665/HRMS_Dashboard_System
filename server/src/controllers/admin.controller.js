...
export const getAllAsset = async (req, res, next) => {
  try {
    let response = await adminServices.getAllAsset();
...
export const deleteAsset = async (req, res, next) => {
  try {
    let response = await adminServices.deleteAssetService(req.params.id);
    if (response.success) {
      return successResponse(res, response, response.message, STATUS.ACCEPTED);
    } else {
      errorResponse(res, response.message, STATUS.BAD_REQUEST);
...
export const updateAsset = async (req, res, next) => {
  try {
    let response = await adminServices.updateAssetService(
      req.params.id,
      req.body
    );
    if (response.success) {
